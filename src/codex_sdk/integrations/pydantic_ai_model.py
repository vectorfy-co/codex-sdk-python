...
class CodexModel(Model):
    ...
    def __init__(
        ...
        *,
        ...
        self._app_server_options: Optional[AppServerOptions] = None
        if self._legacy_codex is None:
            self._app_server_options = self._resolve_app_server_options(
    ...
    async def _ensure_client(self) -> AppServerClient:
        ...
        if self._legacy_codex is not None:
            raise CodexError("Legacy Codex transport does not use app-server client")
    ...
    async def _run_codex_legacy_request(
        self,
        messages: list[ModelMessage],
        model_settings: Optional[ModelSettings],
        model_request_parameters: ModelRequestParameters,
    ) -> tuple[List[Any], RequestUsage, str, ModelRequestParameters]:
        """Run one request through the legacy `codex exec` path."""
        ...
        assert self._legacy_codex is not None
        with span(
            "codex_sdk.pydantic_ai.model_request",
            model=self._thread_options.model,
            sandbox_mode=self._thread_options.sandbox_mode,
            transport="exec_legacy",
        ):
            thread = self._legacy_codex.start_thread(self._thread_options)
        ...
        thread_id = thread.id
        if not isinstance(thread_id, str) or not thread_id:
            raise CodexError("Legacy Codex thread id is unavailable")
    ...
    async def _run_codex_request(
        ...
        """Run one model request via app-server default or explicit legacy Codex."""
        if self._legacy_codex is not None:
            return await self._run_codex_legacy_request(
                messages=messages,
